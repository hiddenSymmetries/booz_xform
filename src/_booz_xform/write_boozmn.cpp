...
void Booz_xform::write_boozmn(std::string filename) {
  ...
  if (phip.size()==0) {
      ...
      phip[0] = 0;
      nc.put(radius_dim, "phip_b", phip, "Derivative of toroidal flux (divided by (-2*pi)) with respect to s. This grid generally does not correspond to the radial grid used for other quantities in this file!", "Tesla * meter^2");
      nc.put(radius_dim, "chi_b", chi, "Poloidal flux (not divided by (2*pi)). This grid generally does not correspond to the radial grid used for other quantities in this file!", "Tesla * meter^2");
